...
class Telegram
{
 ...
 public function sendPoll($chat_id, $question, $options, $disable_notification = false, $reply_to_message_id = null, $reply_markup = null)
  {
    $params = compact('chat_id', 'question', 'options', 'disable_notification', 'reply_to_message_id', 'reply_markup');
  ...
  private function sendRequest($method, $params)
  {
    ...
    // return json_decode(file_get_contents('https://api.telegram.org/bot'.$bot_api_key.'/sendMessage?'.http_build_query($params)), true);
    
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
    curl_setopt($ch, CURLOPT_FOLLOWLOCATION, 1);
    
    curl_setopt($ch, CURLOPT_URL, $this->baseURL . $method . '?' . http_build_query($params));//'https://api.telegram.org/bot'.$bot_api_key.'/sendMessage?'.http_build_query($params));
    ...
    if($data->ok == false) {
        die("An error occured, bot ".$data->description);
        // throw new TelegramException($data->description);
    }else {
        return $data;
    }
